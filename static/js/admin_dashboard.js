...
table_length.on('change', function (e) {
  window.location.replace('/dashboard/magang?length=' + e.target.value);
})

search_karyawan = ()=>{
  $("#dashboard1 #search").val();
  if($("#dashboard1 #search").val() == ""){
    window.location.replace('/dashboard/magang');
  } else {
    window.location.replace('/dashboard/magang?search=' + $("#dashboard1 #search").val());
  }
}
