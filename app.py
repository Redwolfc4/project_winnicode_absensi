...
def signIn():
    ...
    if request.method == "POST":
                ...
                if user['jobs'] in ('Magang','Karyawan'):
                    ...
                    if user["mulai_kerja"] != None and user["akhir_kerja"] != None:
                        ...
                        if not cek_tanggal_kerja(
                            ...
                            raise Exception("Your account expired or now is not your start work")            
                    else:
        ...
        # handle error
            ...
            return jsonify(
                {
                    "redirect": url_for("signIn",msg=e.args[0])
                }
            ),500
...
def forgetPasswordOtp(jwt_otp):
    ...
    if request.method == "POST":
        ...
        # catch erronya
            ...
            return jsonify(redirect=url_for("forgetPassword", msg="Token Kadaluarsa")),500
        except jwt.InvalidTokenError:
            return jsonify(redirect=url_for("forgetPassword", msg="Token TIdak valid")),500
        except ValueError as e:
            return jsonify(
    ...
    try:
        ...
        if (
            ...
            return jsonify(
                {
                    "result": "unsuccess",
                    "redirect": url_for('dashboard',msg="Anda sudah absen pada tanggal "+ riwayat_absen["tanggal_hadir"]),
                }
            ),200
        ...
        # success
        return jsonify({"result": "success", "redirect": "/dashboard/magang"}),200
    # handle error
    except jwt.ExpiredSignatureError:
...
def riwayat_bantuan_post():
    ...
    if not csrf_token or not cookie:
        return jsonify({'redirect':url_for("signIn", msg="convert token failed!")}),500
    try:
        ...
        if not x_request or not x_csrf or x_request != 'XMLHttpRequest':
            return jsonify({'redirect':url_for('signIn', msg="Terjadi kesalahan data")}),500
        ...
        payloads = jwt.decode(cookie, secretKey, algorithms=["HS256"])
        if payloads['role']!=1:
            return jsonify({'redirect':url_for('dashboard',msg="Anda tidak memiliki akses")}),500
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if path1 == 'edit':
                ...
                return redirect(url_for("kelola_admin", msg="Data atas nama "+result['nama']+" success edit", result='success'))
        ...
        if payloads["role"] != 1 and payloads['jobs'] not in ('Admin') and data['departement'] not in ('Superuser'):
            return redirect(url_for('notFound'))
        # cari data admin
...
def kelola_admin_export(path1):
    ...
    try:
        ...
        data_user = db.users.find_one({'_id':ObjectId(payloads['_id'])},{'_id':0,'jobs':1,'role':1,'nama':1,'photo_profile':1,'departement':1})
...
def dashboardAdminCreateAccount():
    ...
    try:
        ...
        data_user = db.users.find_one({'_id': ObjectId(payload['_id'])},{'_id':0,'departement':1,'jobs':1,'role':1})
        
        # cek hak akses selama cookie
        if payload['role'] != 1 and payload['jobs'] not in ('Admin') and data_user['departement'] not in ('Superuser'):
...
def dashboardAdminEdit():
    ...
    try:
        ...
        data_user = db.users.find_one({'_id': ObjectId(payloads['_id'])},{'_id':0,'departement':1,'jobs':1,'role':1})
